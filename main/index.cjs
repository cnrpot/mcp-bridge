...
let win,tray,rt,bridge,tunnel,token,history,exec;let busy=false,quitting=false,pending=0;let activities=[];let saveTimer;
...
async function capture(args={}){const epoch=rt.epoch;require('../core/policy').ensureAllowed('screenshot');if(!args.preConfirmed&&rt.config.screenshotConfirm&&!await confirm({kind:'capture',message:`原因：${args.reason||'未提供'}\n截图会发送给远程 AI，可能包含敏感信息。`}))throw Error('[user refused] Screenshot not taken');require('../core/policy').ensureAllowed('screenshot');
 ...
 if(!selected.length)throw Error('No capturable screen/window matched');return selected.slice(0,4).flatMap(s=>{if(s.thumbnail.isEmpty())throw Error('Screen capture is unavailable or blocked by the OS');return [{type:'text',text:s.name},{type:'image',mimeType:'image/jpeg',data:s.thumbnail.toJPEG(80).toString('base64')}];});}
...
async function action(name,p={}){
 switch(name){
  ...
  case 'settings':{if(!p||typeof p!=='object')throw Error('Invalid settings');const cfg=rt.config;
   ...
   for(const k of ['commandConfirm','screenshotConfirm','editConfirm','closeToTray'])if(typeof p[k]==='boolean')cfg[k]=p[k];
...
app.whenReady().then(async()=>{
 ...
 const defaults=rt.config;rt.config={...defaults,...stored,permissions:{...defaults.permissions,...stored.permissions}};delete rt.config.encryptedToken;
 ...
 if(process.env.MCP_BRIDGE_RESTART_TEST&&process.env.MCPGO_DESKTOP_TEST_DATA&&process.env.MCPGO_DESKTOP_SMOKE_DIR)require('./restart-test.cjs')({win,action,bridge,rt,app});
...
app.on('before-quit',e=>{if(!quitting){e.preventDefault();quitting=true;Promise.resolve(rt?stop():null).finally(()=>{tray?.destroy();app.quit();});}});
