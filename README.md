...
当前源码版本为 1.2.0；首次公开发行版本为 1.1.1。仓库只包含源码、图标、许可和测试脚本，不包含安装包、依赖目录、连接凭据、运行日志或测试数据。下列 release 路径为本地构建输出，并非仓库内置文件。验证报告引用的原始测试数据保留在本地，不公开上传。

## 交付形式

目标平台：Windows x64（Windows 10/11；未逐一验证所有系统版本）。

- `release/mcp-bridge-1.2.0/mcp-bridge-1.2.0-Setup-x64.exe`：安装向导，可选安装路径，带快捷方式和卸载入口。
- `release/mcp-bridge-1.2.0/mcp-bridge-1.2.0-x64.zip`：免安装版。完整解压后运行 `mcp-bridge.exe`。不要只复制 EXE，旁边的 DLL 和 resources 文件夹是运行时的一部分。
- `release/mcp-bridge-1.2.0/win-unpacked/`：未压缩的同一桌面程序。

安装包尚未使用开发者代码签名证书。Windows 可能显示未知发布者或信誉提示；正式公开分发前应配置你自己的代码签名证书。不要关闭系统安全软件。

## 第一次使用

1. 启动 mcp-bridge，点击右上角“选择工作目录”。该目录是默认工作目录；手动/自动审批下也是文件工具的访问边界。
...
5. 按需授权：默认只开启 Read，Edit、Execute、Capture 默认关闭。默认使用手动审批；审批模式与工具权限是两层控制。
...
| 模式 | 文件修改 / 命令 | 截图 | 文件访问范围 |
|---|---|---|---|
| 手动审批（默认） | 每次本地确认 | 每次本地确认 | 所选工作目录 |
| 自动审批 | 已启用的常规操作自动批准；命中风险规则的命令仍确认 | 每次本地确认 | 所选工作目录 |
| 完全访问 | 不逐次确认，开启全部工具权限 | 不逐次确认 | 当前 Windows 用户有权访问的文件，包括工作目录外 |

在连接控制台选择模式。进入自动审批或完全访问需要本机确认，远程 MCP 参数不能代替这一步。完全访问不提权、不绕过 Windows ACL；默认目录仍用于相对路径和搜索。切回其他模式会恢复进入完全访问前的工具开关。模式和已确认的完全访问选择会跨重启保留；旧配置没有模式时使用手动审批。

**自动审批的风险规则不是安全沙箱，也不能识别所有危险脚本。** Execute 在所有模式下都拥有当前 Windows 用户权限。切换模式不会杀死已有命令，需要时先在任务页停止。确认过程中切换模式、目录或停止连接，会拒绝过期操作。仅对可信客户端使用自动审批或完全访问。

## 应用更新

- 默认在启动约 10 秒后及每 6 小时检查 GitHub 的最新稳定 Release；可在设置中关闭，或点击“检查更新”。发现新版后，顶部显示更新入口。
- **只自动检查，不自动下载、不自动安装。** 在设置中点击“下载安装包”，可查看进度、取消和重试。
- 来源固定为 `https://github.com/cnrpot/mcp-bridge`。仅接受匹配版本的 Windows x64 安装包、HTTPS 允许域名、合法大小和 GitHub SHA256 元数据；下载结束和安装前均校验文件。拒绝校验失败的文件并清理下载残片。
- 点击“退出并安装”仍需本地确认，即使当前为完全访问。还有运行任务、工具操作或待审批请求时不允许安装。安装会退出应用并启动普通安装向导，不执行静默安装、不自动提权。开发态不允许启动安装包。
- 免安装版可打开发布页下载 ZIP，退出旧版后完整替换程序目录；点击“退出并安装”会进入安装版向导，不会原地更新 ZIP 目录。
- 首次从 **1.1.1 升级到 1.2.0 需要手动下载安装包或 ZIP**：旧版尚无本功能。更新检查只认识已公开发布的版本，本地构建不会自动成为 GitHub Release。
- SHA256 用于完整性检查，不等同于开发者代码签名。安装包目前未签名；网络错误可稍后重试或打开发布页手动下载，不要关闭系统安全或 TLS 校验。
...
- 手动/自动审批下，文件工具校验所选目录边界、真实路径、符号链接/junction 和硬链接；完全访问允许目录外路径及链接目标，但仍拒绝非法路径、设备路径和 NTFS 数据流。目录遍历跳过链接、依赖和生成目录。搜索在独立 Worker 中执行，受数量、大小、时间限制。
...
- 截图可能包含屏幕上的敏感信息。Capture 默认为关闭；手动/自动审批中逐次确认，完全访问中不再逐次确认。
...
增加三种审批模式、工作目录外完全访问、自动更新检查及手动下载安装；修复远程截图参数伪造内部确认的问题。保留现有配置、凭据和历史目录。详见 `RELEASE_NOTES_1.2.0.md`。
