...
function resolvePath(input){
 ...
 if(require('./approval').mode()!=='full'&&!inside(candidate,root))throw Error('Path is outside the selected workspace');
 ...
 if(require('./approval').mode()!=='full'&&!inside(real,root))throw Error('Symbolic link / junction escapes the selected workspace');
 if(require('./approval').mode()!=='full'&&fs.statSync(existing).isFile() && fs.statSync(existing).nlink>1)throw Error('Hard-linked files are not supported by workspace tools');
 return candidate;
}
